...
//            addQuests(HiddenCollection.generate());
            ...
            System.out.println("Java DB Driver problem - cannot create DB: "
                    + ex.getMessage());
        }
    }
    ...
    private LinkedList<Score> getScores(Player player) throws SQLException {
        ...
        String SQL
                = "SELECT * FROM SCORES, PLAYERS, GAMES WHERE PLAYERS.ID = "
                + "SCORES.PLAYER AND GAMES.ID = SCORES.GAME AND "
                + "PLAYERS.PLAYER = '" + player.getName() + "'";
        ResultSet results = getDataRecords(SQL);
        while (results.next()) {
            String game = results.getString("GAME");
            Date date = results.getDate("DATE");
            int points = results.getInt("POINTS");
            int comp = results.getInt("COMPETION_TIME");
    ...
    public LinkedList<Score> getScores(String game) throws SQLException {
        LinkedList<Score> scores = new LinkedList<>();
        String SQL
                = "SELECT MISSED_CLICKS, COMPLETION_PROGRESS, LEVEL, COMPETION_TIME, DATE, POINTS, PLAYERS.PLAYER FROM SCORES, GAMES, PLAYERS WHERE "
                + "SCORES.GAME = GAMES.ID AND SCORES.PLAYER = PLAYERS.ID "
                + "AND GAMES.GAME = '" + game + "'";
        ResultSet results = getDataRecords(SQL);
        while (results.next()) {
    ...
    public LinkedList<Score> getScores(Player player, String game) throws
            SQLException {
        LinkedList<Score> scores = new LinkedList<>();
        String SQL
                = "SELECT * FROM SCORES, PLAYERS, GAMES WHERE PLAYERS.ID = "
                + "SCORES.PLAYER AND GAMES.ID = SCORES.GAME AND GAMES.GAME ='"
                + game + "' AND PLAYERS.PLAYER = '" + player.getName() + "'";
        ResultSet results = getDataRecords(SQL);
        while (results.next()) {
            Date date = results.getDate("DATE");
            int points = results.getInt("POINTS");
    ...
    public void printRecords(Player player) throws IOException {
        ...
        try {
            ...
            //String otherFolder = userHomeFolder + File.separator + "Desktop" + File.separator + "Scores";
            ...
            for (Score score : scores) {
                ...
                i++;
                  ...
                  /*bw.newLine();
                  bw.append(gamename + ",");
                  bw.append(score.getPoints()+ ",");
                  bw.append(score.getDate() + ",");
                  bw.append(score.getCompTime()+ ",");
                  bw.append(score.getLevel() + ",");
                  bw.append(String.valueOf(score.getProgress()*100) + ",");
                  bw.append(String.valueOf(score.getMissedClicks()));
                  bw.newLine();*/
            }
            wb.write(fileOut);
            fileOut.close();
    ...
    public void printRecordsGame(String game) throws IOException {
        ...
        try {
            ...
            for (Score score : scores) {
                  ...
                  String playername = "";
                  while (results.next()) {
                   playername = results.getString("PLAYER");
                  }*/
                ...
                i++;
                  ...
                  /*bw.newLine();
                  bw.append(score.getPlayer().getName()+ ",");
                  bw.append(score.getPoints()+ ",");
                  bw.append(score.getDate() + ",");
                  bw.append(score.getCompTime()+ ",");
                  bw.append(score.getLevel() + ",");
    ...
    public void recordScore(Player player, String game, int points, int compTime, int lvl, double compProg, int missedClicks) {
        try {
            ...
            if (playerID > 0 && gameID > 0) {
                insert("SCORES", "PLAYER, GAME, DATE, POINTS, COMPETION_TIME, LEVEL, COMPLETION_PROGRESS, MISSED_CLICKS", playerID
                        + ", " + gameID + ", '" + sqlDate + "', " + points + ", " + compTime + ", " + lvl + ", " + compProg + ", " + missedClicks);
    ...
    public void recordPlayer(Player player) {
        insert("PLAYERS", "PLAYER", "'" + player.getName() + "'");
    }
    ...
    public void renamePlayer(String oldName, String newName) {
        try {
            String SQL = "UPDATE PLAYERS SET PLAYER = '" + newName
            ...
            if (id > 0) {
                SQL = "UPDATE SCORES SET PLAYER = '" + newName
                        + "' WHERE ID = '" + id + "'";
                System.out.println(SQL);
                conn.createStatement().executeUpdate(SQL);
            }
